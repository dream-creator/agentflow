...
| PayMongo webhook signature | Prevents forged webhook events | `src/app/api/paymongo/webhook/route.ts` |
...
| --- | --- |
| PayMongo secret | PayMongo dashboard → API keys → roll. Update Vercel. |
| PayMongo webhook secret | PayMongo dashboard → webhooks → endpoint → roll. Update Vercel. |
...
- [ENVIRONMENT-VARIABLES.md](../getting-started/environment-variables.md) — the
  full env matrix.
